...
import com.sumeet.cars360.databinding.FragmentInvoicesUploadBinding
...
class InvoicesUploadFragment : Fragment(),OnServiceItemClickListener{

    private lateinit var binding: FragmentInvoicesUploadBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentInvoicesUploadBinding.inflate(inflater, container, false)
        return binding.root
    }
            ...
            when(it){
                is Resource.Loading -> {
                    ViewVisibilityUtil.visibilityExchanger(visible = binding.progressBar
                        ,gone = binding.customerInvoiceUploadRecyclerview)
                    ViewVisibilityUtil.gone(binding.errorMessage)
                }
                is Resource.Error -> {
                    ViewVisibilityUtil.gone(binding.customerInvoiceUploadRecyclerview)
                ...
                is Resource.Success -> {
                    ViewVisibilityUtil.visibilityExchanger(visible = binding.customerInvoiceUploadRecyclerview
                        ,gone = binding.progressBar)
                    ViewVisibilityUtil.gone(binding.errorMessage)
                    binding.customerInvoiceUploadRecyclerview.apply {
                        layoutManager = LinearLayoutManager(context)
                        adapter = it.data?.serviceLogResponse?.let { data ->
                            ServiceLogRecyclerAdapter(
                                data,
                                this@InvoicesUploadFragment
                            )
                        }
                    }
                }
